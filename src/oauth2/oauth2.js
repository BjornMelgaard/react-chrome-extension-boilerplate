 ...
 */
OAuth2.prototype.getAccessAndRefreshTokens = function(
  authorizationCode,
  callback
) {
  var that = this
  ...
  var items = that.adapter.accessTokenParams(
    authorizationCode,
    that.getConfig()
  )
  var key = null
  if (method == 'POST') {
    ...
    for (key in items) {
      params +=
        encodeURIComponent(key) + '=' + encodeURIComponent(items[key]) + '&'
    }
...
OAuth2.prototype.finishAuth = function() {
  ...
  try {
    authorizationCode = that.adapter.parseAuthorizationCode(
